...
This repository now includes `vercel.json` and `api/index.ts` for serverless deployment.

1. Import this repository in Vercel.
2. Keep install/build commands as:
...
3. Add these environment variables in Vercel project settings:
   ...
   - `JWT_SECRET`
   - `VITE_APP_ID`
   - `OAUTH_SERVER_URL` (usually `https://oauth.manus.im`)
   - `VITE_OAUTH_PORTAL_URL` (usually `https://app.manus.im`)
...
This repository now includes `Dockerfile` and `.dockerignore`.
